...
/**
 * This component represents an SVG canvas for rendering maps with nodes and edges.
 * @param {Nodes[]} props.path - Array of nodes representing the path to be highlighted.
 * @param {string} props.currentMap - The current map being displayed.
 * @param {(map: string) => void} props.setCurrentMap - Function to set the current map.
 * @param {string} props.currentLevel - The current level of the map.
 * @param {string} props.nodeColor - Optional color for rendering nodes.
 * @param {string} props.edgeColor - Optional color for rendering edges.
 * @param {Nodes | undefined} props.nodeClicked - The currently clicked node.
 * @param {(node: Nodes | undefined) => void} props.handleNodeClicked - Function to handle node clicks.
 * @param {Edges | undefined} props.edgeClicked - The currently clicked edge.
 * @param {(edge: Edges | undefined) => void} props.handleEdgeClicked - Function to handle edge clicks.
 * @param {(node: Nodes | undefined) => void} props.handleNodeHover - Function to handle node hover events.
 * @param {boolean} props.isHome - Indicates whether the canvas is being used in the home page.
 * @param {boolean} props.showPathOnly - Indicates whether to show only the path or the entire map.
 * @param {Nodes[]} props.allnodes - Array of all nodes in the map.
 * @returns {JSX.Element} Returns the SVG canvas component.
 */
export default function SVGCanvas(props: {
  ...
  // useEffect hook responsible for setting current Floor to the currentLevel whenever currentLevel changes
  useEffect(() => {
   ...
   * It determines whether to display a message indicating whether to go up or down
  ...
  function getTypePopup(node: Nodes, path: Nodes[]): number {
    ...
    // return no floor change by default
    return 0;
  }
  ...
  /**
   * This function handles clicks on nodes in the SVG canvas.
   * It invokes the handleEdgeClicked callback with the clicked edge as an argument,
   * and resets the node clicked state by invoking the handleNodeClicked callback with undefined.
   * @param {Edges} edge - The clicked edge.
   */
   ...
   * @type {Nodes[]} filteredNodes - Array of nodes that meet the filtering criteria.
   */
  ...
  //This variable filters the edges data based on the availability of start and end nodes in the filteredNodes array.
  ...
  return (
    ...
    >
      ...
      {filteredNodes.map(
        (
          node, // Map over each node in the filteredNodes array
        ) => (
          <g>
            {" "}
            // Render a group element for each node
            ...
            (getTypePopup(node, props.path!) == 1 || // Check if the current node is an elevator or stairs
              getTypePopup(node, props.path!) == -1) ? ( // Check if the current node is a relevant elevator or stairs
              <g>
                {" "}
                // Render a nested group element for elevator or stairs nodes
                <circle // Render a circle for the node
                  r="15" // Set the radius of the circle
                  cx={+node.Xcoord} // Set the x-coordinate of the circle center
                  cy={+node.Ycoord} // Set the y-coordinate of the circle center
                  fill={getNodeColor(node)} // Set the fill color of the circle based on node properties
                />
                <image // Render an image for elevator or stairs icon
                  onMouseOver={() => handleElevatorHover(node, props.path!)} // Handle mouse over event for elevator or stairs icon
                  onClick={() =>
                    // Handle click event for elevator or stairs icon
                    handleElevatorClick(
                      node,
                  ...
                  key={node.NodeID} // Set a unique key for the image
                  href={ElevatorIcon} // Set the image source
                  x={+node.Xcoord - 30} // Set the x-coordinate of the image position
                  y={+node.Ycoord - 30} // Set the y-coordinate of the image position
                  width="60" // Set the width of the image
                  height="60" // Set the height of the image
                />
              </g>
            ) : (
              // Render circle for regular nodes if conditions are not met
              <circle
                onClick={() => handleNodeClick(node)} // Handle click event for regular nodes
                onMouseEnter={() =>
                  // Handle mouse enter event for regular nodes
                  props.handleNodeHover && props.handleNodeHover(node)
                }
                onMouseLeave={() =>
                  // Handle mouse leave event for regular nodes
                  props.handleNodeHover && props.handleNodeHover(undefined)
                }
                cx={node.Xcoord} // Set the x-coordinate of the circle center
                cy={node.Ycoord} // Set the y-coordinate of the circle center
                r="10" // Set the radius of the circle
                fill={props.nodeColor ?? getNodeColor(node)} // Set the fill color of the circle based on node properties or props.nodeColor
              />
            )}
          </g>
        ),
