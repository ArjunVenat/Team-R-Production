    ...
    async function draw(ctx: CanvasRenderingContext2D) {
        ...
        if (nodesData && props.path) {
            ...
            for (let i = 0; i < props.path!.length - 1; i++) {
                ...
                ctx.lineWidth = 5;
                ctx.lineTo(Number(props.path![i + 1].Xcoord), Number(props.path![i + 1].Ycoord));
                ctx.strokeStyle = 'blue';
                ctx.stroke();

                const angle = Math.atan2(Number(props.path![i + 1].Ycoord) - Number(node.Ycoord), Number(props.path![i + 1].Xcoord) - Number(node.Xcoord));
